...
public class ToggleHeight : MonoBehaviour
{
    ...
    [Header("Boundaries of the map")] [SerializeField]
    ...
    private void toggleHeight()
    {
        if (highPosition)
        {
            ...
            transform.position = new Vector3(Math.Clamp(intersectionPoint.x, minX, maxX), minHeight,
                Math.Clamp(intersectionPoint.z, minY, maxY));
        }
        else
        {
